...
class ContrastiveHead(BaseModule):
    ...
    def forward(self, x: Tensor, w: Tensor) -> Tensor:
        ...
        if self.use_einsum:
            ...
            # 变换 x 的形状以适应矩阵乘法
            x_reshaped = x.view(x.size(0), x.size(1), -1).permute(0, 2, 1)  # (B, H*W, C)

            # 转置 w 以适配矩阵乘法
            w_transposed = w.permute(0, 2, 1)  # (B, C, K)

            # 执行矩阵乘法
            output = torch.matmul(x_reshaped, w_transposed)  # (B, H*W, K)

            # 恢复形状
            x = output.permute(0, 2, 1).view(x.size(0), w.size(1), x.size(2), x.size(3))  # (B, K, H, W)
    ...
    def forward(self, x: Tensor, w: Tensor) -> Tensor:
        ...
        if self.use_einsum:
            x = torch.einsum('bchw,bkc->bkhw', x, w)
             ...
             # 重塑张量以便进行矩阵乘法
            x_reshaped = x.permute(0, 2, 3, 1).reshape(x.size(0), -1, x.size(1))  # (B, H*W, C)
            w_transposed = w.permute(0, 2, 1)  # (B, C, K)
            ...
            # 恢复形状
            x = output.permute(0, 2, 1).reshape(x.size(0), w.size(1), x.size(2), x.size(3))  # (B, K, H, W)
        ###############
        x = x * self.logit_scale.exp() + self.bias
        return x
...
class YOLOWorldHead(YOLOv8Head):
    ...
    def predict(self,
        ...
        ########
        # data = outs
        # torch.save(data, "predict_head_predict_forword.pt")
        # data = predictions
        # torch.save(data, "predict_head_predict.pt")
        ########
        return predictions
    ...
    # def my_bbox_post_process(self, cls_scores: Tensor,
    #                     bbox_preds: Tensor,
    ...
    #     flatten_cls_scores = cls_scores.reshape(1, 1, )
    #     flatten_bbox_preds = bbox_preds
    #     #######
    #     # data = {"scores":flatten_cls_scores, "bbox":flatten_bbox_preds,"objectnesses":objectnesses}
    #     # torch.save(data, "runner_before_deocde.pt")
    #     #######
    #     flatten_decoded_bboxes = self.bbox_coder.decode(
    #         flatten_priors[None], flatten_bbox_preds, flatten_stride)
    #     #######
    #     # data = flatten_decoded_bboxes
    #     # torch.save(data, "runner_after_deocde.pt")
    #     #######
    ...
    def predict_by_feat(self,
        ...
        ########
        # data = {"flatten_stride":flatten_stride, "flatten_cls_scores":flatten_cls_scores, "flatten_bbox_preds":flatten_bbox_preds, "mlvl_priors":self.mlvl_priors}
        # torch.save(data, "predict_head_predict_by_feat.pt")
        ########
        flatten_cls_scores = torch.cat(flatten_cls_scores, dim=1).sigmoid()
        flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1)
        #######
